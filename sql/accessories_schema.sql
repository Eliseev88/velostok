-- Аксессуары для велосипедов с velosklad.ru.
...
-- Применение: mysql -u root velostok < sql/accessories_schema.sql
...
CREATE TABLE IF NOT EXISTS accessory_specs (
  ...
  value        TEXT NULL,
  PRIMARY KEY (accessory_id, position),
  KEY idx_name (name),
  CONSTRAINT fk_aspecs_acc FOREIGN KEY (accessory_id) REFERENCES accessories(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS accessory_images (
  ...
  PRIMARY KEY (accessory_id, position),
  CONSTRAINT fk_aimages_acc FOREIGN KEY (accessory_id) REFERENCES accessories(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS accessories_queue (
  accessory_id BIGINT       NOT NULL PRIMARY KEY,
  url          VARCHAR(512) NOT NULL,
